...
So there is a fantastic insight in this equation: the market price implies a certain discount rate, which we can use to understand the risk and expected return of the stock. We can understand the premiums provided by lower relative price, higher profitability, and more conservative investment from a theoretical perspective. And once we do that, we see the crucial phrase I used repeatedly: all else equal. Ceteris paribus! This is essential because it means that attempting to achieve higher returns by selecting for stocks with low relative prices, or high profitability, or conservative investment is less effective than selecting for stocks with all of those characteristics simulataneously.

This has led to a proliferation of multifactor funds. Unfortunately, I currently have no solid basis to believe that most of them will achieve consistently high exposure to the academic risk factors. Many fund managers are not as concerned with rigorously capturing risk premiums as they are with marketing their funds as being based on fashionable factors. The Avantis value funds, which filter for profitability _as well as_ low relative price, are my main recommendation for factor tilts. One example of a specific portfolio using these funds is provided above (complex portfolio A). [↩](#fn2)
...
There are other [multifactor funds](https://www.msci.com/msci-diversified-multi-factor-indexes) which I will list here. You might consider using these funds to achieve risk factor diversification more simply, although the methods of diversification are less clear than when implemented on your own. Expense ratios are slightly higher but still reasonable.
